...
- `POST /shorten`: recebe `{ "url": "https://mbzerker.github.io/nBTChat/..." }`, link do CompraLink ou link do Caderno de Receitas e devolve `{ shortUrl }`.
...
O encurtador aceita apenas destinos conhecidos dos projetos `nBTChat`, `CompraLink` e `CadernoReceitas` no GitHub Pages, alem das paginas internas de compra/recuperacao do proprio Worker. Isso evita que o dominio seja usado como encurtador aberto para links de terceiros.
